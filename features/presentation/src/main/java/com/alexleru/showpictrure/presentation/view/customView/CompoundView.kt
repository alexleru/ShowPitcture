...
import com.alexleru.showpictrure.presentation.databinding.ImageCompoundViewBinding
...
class CompoundView @JvmOverloads constructor(
    ...
    defStyleAttr: Int = common.uikit.R.attr.viewStyleCompoundViewDefault,
    defStyleRes: Int = 0
) : ConstraintLayout(
    ...
    init {
        ...
        getContext().withStyledAttributes(
            attrs,
            common.uikit.R.styleable.CompoundView,
            defStyleAttr,
            defStyleRes
        ) {
            textDate = getString(common.uikit.R.styleable.CompoundView_textCompound).orEmpty()
            sizeRelativeToWidthCompound =
                getInt(
                    common.uikit.R.styleable.CompoundView_sizeRelativeToWidthCompound,
                    MINIMAL_TEXT_WIDTH_PERCENT_OF_IMAGE_VIEW
                )
            getColor(
                common.uikit.R.styleable.CompoundView_backgroundTextCompound,
                Color.BLACK
            ).run {
                bind.textViewCompound.setBackgroundColor(this)
            }

            getColor(common.uikit.R.styleable.CompoundView_textColorCompound, Color.BLACK).run {
                bind.textViewCompound.setTextColor(this)
            }

            getDrawable(common.uikit.R.styleable.CompoundView_imageCompound).run {
